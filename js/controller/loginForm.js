(function(){
  angular.module("buscoPiso").controller("LoginController", ['$scope', '$http', '$translate', 'Domain', function($scope, $http, $translate, Domain) {
    ...
    this.login = function () {
      ...
      $http.post(Domain + 'api/public/users/login/', {"email" : $scope.email, "password" : $scope.password}).then(function (response) {
        if (response.data.length>0) {
          ...
          $scope.flag = true;
          $window.open('clientMainPage.html', "_self");
          return $scope.flag;
        }
        else{
          $scope.flag = false;
